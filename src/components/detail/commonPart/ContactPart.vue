<template>
  ...
  <div :class="{contacts_part : hasClass}">
    <div class="info_title">
      <span class="title">往来信息</span>
    </div>
    <div class="main_content vux-1px contact_whole">
      <div class="each_info">
        <span class="title vux-1px-r">往来名称</span>
        <span class="content">{{contactInfo.dealerName}}</span>
      </div>
      <div class="each_info">
        <span class="title vux-1px-r">地址</span>
        <span class="content" v-if="!noAddress">{{contactInfo.province}}{{contactInfo.city}}{{contactInfo.county}}{{contactInfo.address}}</span>
        <span class="content" v-else>暂无</span>
      </div>
      <div class="each_info" v-show="showAllContact">
        <span class="title vux-1px-r">联系人</span>
        <span class="content">{{contactInfo.dealerContactPersonName || '暂无'}}</span>
      </div>
      <div class="each_info" v-show="showAllContact">
        <span class="title vux-1px-r">手机号</span>

  ...
  methods: {
    ...
    handlerSetMatters(callback) {
      let params = {
        ...
        filter: JSON.stringify([
          {
            operator: "like",
            value: this.scanCodeInfo.postCode,
            property: "transCode"
          }
        ])
      };
    ...
    handlerSetMattersBox() {
      ...
      let [
        ...
        batchNo,
        boxRule
      ] = this.scanCodeInfo.boxCode.split("-");
      ...
      if (
        ...
        this.$vux.confirm.show({
          content:
            "当前扫的箱码与前面扫的箱码所属的申请单号不一致，是否切换单据数据，以重新获取待上架数据？",
          // 确定回调
          onConfirm: () => {
            this.matters = [];
        ...
        inPut.dataSet.map(box => {
          if (!materielMap[box.transObjCode]) {
            materielMap[box.transObjCode] = {
              ...
              warehouseName_storehouseInCode:
                box.warehouseName_storehouseInCode,

...
export default {
  ...
  methods: {
    // 选中的客户
    selDealer(val) {
      let dealerConfig = this.dealerConfig;
      let [ dealerInfo = {} ] = JSON.parse(val);
      // 重新选择客户之后 初始化 <物料列表>
      ...
      // 此处为做校验 重组一份数据
      this.dealerInfo = {
        dealerCodeCredit: dealerInfo.dealerCode,            // 供应商编码
        crDealerPaymentTerm: dealerInfo.paymentTerm,        // 结算方式
        pamentDays_dealerCodeCredit: dealerInfo.pamentDays, // 账期天数
        dealerName_dealerCodeCredit: dealerInfo.dealerName, // 供应商名称
        ...dealerInfo
      }
    },
    // 选中联系人
          ...
          let submitData = {
            ...
            formData: JSON.stringify({
              ...
              order: {
                ...
                crDealerLabel: this.dealerInfo.dealerLabelName, // 往来标签
                expectedPaymentDate: this.dealerInfo.expectedPaymentDate,  // 预期付款日            
                crDealerPaymentTerm: this.dealerInfo.crDealerPaymentTerm, // 结算方式
                pamentDays_dealerCodeCredit: this.dealerInfo.pamentDays_dealerCodeCredit, // 账期天书
              },
            }),
    ...
    getFormData() {
      return getSOList({
        ...
        this.dealerInfo = {
          ...
          pamentDays_dealerCodeCredit: formData.order.pamentDays_dealerCodeCredit,
  ...
  updated() {
    ...
    // 若存在缓存 此处对页面进行赋值
    let draft = JSON.parse(sessionStorage.getItem('ZZSSP_DATA'));

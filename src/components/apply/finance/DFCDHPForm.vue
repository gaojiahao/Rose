  ...
  <div class="pages fgysqk-apply-container">
    <div class="basicPart" ref='fill'>
      <div class='fill_wrapper'>
          ...
          <template v-else>
            ...
            <div class="order-detail" :class="{'vux-1px-t': index !== 0}" v-for="(item, index) in orderList"
              ...
              <div class="detail-item">
                ...
                <span class="info-item">保证金: {{item.deposit | numberComma}}</span>
              </div>
              <div class="detail-item">
        ...
        <pop-cash-list :default-value="cashInfo" @sel-item="selCash" request="3" :required="true">
          <template slot="other">
            <x-input class="amt-cash" title="支付金额" text-align='right' placeholder='请填写' type='number'
                     @on-blur="checkAmt(cashInfo)" @on-focus="getFocus($event)" v-model.number='cashInfo.tdAmount'></x-input>
          </template>
        </pop-cash-list>
        <div class="materiel_list">
        ...
        <!-- 采购单信息 -->
        <pop-money-order-list :show="showOrder" :default-value="orderList" @sel-item="selOrder" request="2"
                              v-model="showOrder" get-list-method="getDraftBalance">
          <template slot="amount" slot-scope="{item}">
            <span>金额: {{item.amntTbMatched | numberComma}}</span>
            <span>保证金: {{item.deposit | numberComma}}</span>
          </template>
    ...
    methods: {
      ...
      submitOrder() {
        ...
        this.$vux.confirm.show({
          ...
          onConfirm: () => {
            ...
            let wfPara = {
              [this.processCode]: {businessKey: "HATB", createdBy: JSON.stringify(this.formData.handler)}
            };
            if (this.isResubmit) {
              wfPara = {
      ...
      getFormData() {
        return getSOList({
          ...
          let {dataSet = []} = order;
          this.cashInfo = {
            fundName: outPut.dataSet[0].fundName_cashOutCode,
            fundCode: outPut.dataSet[0].cashOutCode,
            fundType: outPut.dataSet[0].cashType_cashOutCode,
            thenAmntBal: outPut.dataSet[0].thenAmntBal,
            amntBal: outPut.dataSet[0].thenAmntBal,
            tdAmount: outPut.dataSet[0].tdAmount,
          }

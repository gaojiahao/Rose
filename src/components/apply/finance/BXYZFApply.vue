  ...
  <div class="pages sj-apply-container">
    <div class="basicPart no_count" ref="fill">
      <div class="fill_wrapper">
        <pop-baseinfo
          :defaultValue="handlerDefault"
          @sel-item="selItem"
          :handle-org-list="handleORG"
        ...
        <div class="project_part">
          <div
            :class="{'vux-1px-t': dIndex>0}"
            v-for="(dItem,dIndex) in baseinfoExtConfig"
            :key="dIndex"
          >
            <!-- 可编辑的字段 -->
            <template v-if="!dItem.readOnly">
              <!-- 下拉框 -->
              <r-picker
                :title="dItem.fieldLabel"
                :data="dItem.remoteData"
                :value="project[dItem.fieldCode]"
                v-model="project[dItem.fieldCode]"
                :required="!dItem.allowBlank"
                v-if="dItem.xtype === 'r2Selector'"
              ></r-picker>
            </template>
            <!--不可编辑的字段 -->
            <template v-else>
        ...
        <div class="cost_list">
          <div
            class="cost_item"
            :class="{'has_border' : sIndex > 0}"
            v-for="(sItem, sIndex) in costList"
            :key="sIndex"
          >
            ...
            <div
              v-for="(dItem,dIndex) in matterEditConfig.editPart"
              :key="dIndex"
              :class="{'vux-1px-b': dIndex < matterEditConfig.editPart.length-1}"
            >
              <!-- 可编辑的字段 -->
              <template v-if="!dItem.readOnly">
                <!-- 下拉框Plus -->
                <div
                  class="each_property"
                  v-if="dItem.editorType === 'r2SelectorPlus'"
                  @click="getCost(sItem,sIndex)"
                >
                ...
                <pop-fybx-list
                  :show="showCostPop"
                  v-model="showCostPop"
                  @sel-matter="selMatter"
                  :defaultValue="selectedCost"
                  :matter-params="glParams"
                  ref="matter"
                ></pop-fybx-list>
                <!-- 下拉框 -->
                <div
                  class="each_property"
                  v-if="dItem.editorType === 'r2Selector'"
                  @click="getDep(sItem,sIndex)"
                >
                  <label :class="{required: !dItem.allowBlank}">{{dItem.text}}</label>
                  <div class="picker">
                    <span class="mater_nature">{{sItem[dItem.fieldCode] || "请选择"}}</span>
                ...
                <div class="each_property" v-if="dItem.editorType === 'r2Numberfield'">
                  ...
                  <input
                    type="number"
                    v-model.number="sItem[dItem.fieldCode]"
                    placeholder="请输入"
                    class="property_val"
                    @focus="getFocus($event)"
                    @blur="checkAmt(sItem, dItem.fieldCode, sItem[dItem.fieldCode]) "
                  >
                </div>
                <!-- 输入框（文字） -->
                ...
                <div
                  class="each_property"
                  v-if="dItem.editorType === 'r2Datefield'"
                  @click="getDate(sItem,dItem)"
                >
                  <label :class="{required: !dItem.allowBlank}">{{dItem.text}}</label>
                  <div class="picker">
                    <span class="mater_nature">{{sItem[dItem.fieldCode] || "请选择"}}</span>
          ...
          <div
            class="vux-1px-t"
            :class="[this.actions.includes('stop') ? 'resubmit-part' : 'sumbit-part']"
          >
        ...
        <upload-file
          ...
          :biReferenceId="biReferenceId"
        ></upload-file>
      </div>
    </div>
    <op-button
      :is-modify="matterModifyClass"
      :hide="btnIsHide"
      :td-amount="tdAmount"
...
export default {
  ...
  computed: {
    ...
    differenceAmount() {
      let total = 0,
        subAmount = accSub(
          this.dealerInfo.thenAmntBal,
          this.dealerInfo.thenAlreadyAmnt
        );
      ...
      handler(newVal, oldVal) {
        ...
        if (oldVal) {
          ...
          for (let i = 0; i < this.costList.length; i++) {
            this.costList[i].departmentName =
              this.$refs.baseChild.group || this.handlerDefault.handlerUnitName;
          }
        }
        for (let i = 0; i < this.costList.length; i++) {
          this.costList[i].departmentName =
            ...
            if (item.fieldCode === "project") {
              for (let dItem of item.remoteData) {
                if (dItem.name === newVal.project) {
                  ...
                  this.project.dealerName_projectManager =
                    dItem.projectManagerName;
                  ...
                  for (let i = 0; i < this.costList.length; i++) {
                    this.costList[i].departmentName =
                      this.$refs.baseChild.group ||
                      this.handlerDefault.handlerUnitName;
                  }
                  break;
                }
              }
    ...
    submitOrder() {
      ...
      for (let item of this.costList) {
        ...
        for (let sItem of this.submitMatterField) {
          ...
          if (
            sItem.fieldCode != "project" &&
            sItem.fieldCode != "tdProjectId_project" &&
            sItem.fieldCode != "dealerName_projectManager" &&
            sItem.fieldCode != "expectStartDate_project" &&
            sItem.fieldCode != "expectEndDate_project" &&
            sItem.fieldCode != "projectAddress_project" &&
            sItem.fieldCode != "budgetIncome_project"
          ) {
            oItem[sItem.fieldCode] =
              item[sItem.fieldCode] != null ? item[sItem.fieldCode] : null;
          }
        }
        dataSet.push(oItem);
      }

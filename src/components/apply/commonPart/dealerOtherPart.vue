<template>
  <div class="dealer-other-part" v-if="config.length">
    <div v-for="(item,index) in config" :key="index">
      <!-- 结算方式, 物流条款 -->
      <pop-single-select :title="item.fieldLabel" :data="item.rometeData" :value="dealerInfo[item.fieldCode]" v-model="dealerInfo[item.fieldCode]" 
      ...
      <div class="mg_auto">
        ...
        <x-input class="cell-item" text-align='right' placeholder='请填写'
                v-model.number='dealerInfo[item.fieldCode]' v-if="!item.hiddenInRun && item.xtype === 'r2Textfield'">
          <span slot="label">{{item.fieldLabel}}</span>
        </x-input>

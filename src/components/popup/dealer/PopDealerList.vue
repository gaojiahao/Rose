  ...
  <div class="pop_dealer_list">
      ...
      <div class='user-content' v-if="selInfo.dealerCode">
        <div class="user-info">
          <div class="user-name">
            ...
            <span>{{selInfo.nickname}}</span>
          </div>
        </div>
        <div class="cp-info">
          <span>编码: {{selInfo.dealerCode}}</span>
          <span>余额: {{selInfo.amntBal}}</span>
        </div>
      </div>
      <div class='user-content' v-else>
        <div class="title">往来信息</div>
        <div class="required">请选择往来</div>
        <!-- <div>请选择资金信息</div> -->
      </div>
    ...
    <r-popup :show="showDealerPop" :data="listData" :options="scrollOptions" :has-next="hasNext" :default-value="defaultValue"
      ...
      <template slot-scope="{item}" slot="pop-item">
        ...
        <div class="amt">
          <span>往来编码: {{item.dealerCode}}</span>
          <span>往来余额: {{item.amntBal}}</span>
        </div>
      </template>
    </r-popup>
  </div>
...
<script>
  ...
  import {getEmployeeReserveFunds, getSupplierToPayDeposit, getFundCashDiscount, getObjFundBalByType, getComplexExpenseReimbursement, getEmployeeBalList} from 'service/costService'
  ...
  export default {
    ...
    created() {
      let serviceList = [getEmployeeReserveFunds, getSupplierToPayDeposit, getFundCashDiscount, getObjFundBalByType, getComplexExpenseReimbursement, getEmployeeBalList];

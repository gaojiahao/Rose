<template>
  <div class="techics-list-container" @click="showPop = !showPop">
    <div class="picker">
      <label :class='{ required : required}'>{{itemData.fieldLabel}}</label>
      <div class='content'>
        <span class='mater_nature'>{{selItems[itemData.fieldCode] || selItems.dealerName || "请选择"}}</span>
      ...
      <popup v-model="showPop" height="80%" class="popup-technics-list-container" @on-show="onShow" @on-hide="onHide">
        ...
        <r-scroll class="list-container" :options="scrollOptions" :has-next="hasNext"
          ...
          <div class="mater-list-item" v-for="(item, index) in listData" :key='index'
            ...
            <div class="info-item">
              <div class="name">{{item[itemData.fieldCode] || item.dealerName}}</div>
            </div>
            <!-- icon -->
...
<script>
  ...
  import {getUsingProcedure} from 'service/materService'
  import RScroll from 'plugins/scroll/RScroll'
  import RSearch from 'components/search/search'
  import $flyio from 'plugins/ajax'

  export default {
    name: "PopProcedureList",
    props: {
      // 默认值
      defaultValue: {
        type: Object,
    ...
    methods: {
      ...
      getList() {
        ...
        return $flyio.ajax({
            url: this.itemData.dataSource.data.url,
            data

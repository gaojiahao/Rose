...
<script>
  ...
  import {getObjWorkshopWarehouse, getWareHouseType, getWarehouseManager, getObjWarehouseInfo} from 'service/listService'
  ...
  export default {
    ...
    methods: {
      ...
      setDefaultValue() {
        ...
        if(this.selItems.warehouseCode) {
    ...
    created() {
        //let serviceList = [getObjWarehouseInfo];
        let serviceList = {
            getObjWarehouseInfo: getObjWarehouseInfo
        }
        this.requestMethods = serviceList[this.ckParams];
      this.setDefaultValue();
      if (this.disabled) {
        return
      }

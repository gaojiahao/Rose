...
export default {
    ...
    methods: {
        ...
        addRecords: async function (selection) {
            ...
            for (i; i < l; i++) {
                row = selection[i];
                await this.initBomData(row.matCode);
                var bom = [];
        ...
        async initBomDataStorageSum(bomData,flag){
            ...
            for(var i = 0; i < bomData.length; i++){
                for(var j = 0 ; j < bomData[i].length;j++){
                    ...
                    if(this.form.formData['containerCodeOut']){
                        data['whCode'] = this.form.formData['containerCodeOut'];
                    }
            ...
            async function deal(data){
                await getProcessDispatchPickingBom(data).then(res=>{ 

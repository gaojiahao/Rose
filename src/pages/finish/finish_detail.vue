    ...
    <div id="finish_detail">
		...
		<div class="wrapper" v-if="detailInfo.baseinfo" >
			...
			<group label-width="4.5em" label-margin-right="2em" label-align="left" v-if="(detailInfo.transCode).indexOf('CPXQ')>=0">
				...
				<cell title="需求维度" :value="detailInfo.requirement.requireDemension" value-align="left" v-if="detailInfo.requirement.requireDemension.text==''"></cell>						
				<cell title="优先级" :value="detailInfo.requirement.level.text" value-align="left" v-if="detailInfo.requirement.level.text"></cell>
...
import { Group, Cell,XDialog,Icon,CellBox,Flow, FlowState, FlowLine  } from 'vux'
export default{
	data(){
		return{
			infoList:[],
...
#finish_detail .allInfo:last-child .weui-wepay-flow__line{
	display: none;
}
.info p{
	display: block;
